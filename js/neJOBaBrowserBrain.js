...
function ListExtractor(c,a,b){
    ...
    */
    this.showBookmark = function () {
        ...
        var absltURI = document.baseURI + urlParams;
        var rsltURI = encodeURI(absltURI);

        var fcBkUrl = encodeURI('http://www.facebook.com/sharer.php?u=' + rsltURI);
        var gsprUrl = encodeURI('http://sharetodiaspora.github.io/?title=' + document.title + "&url=" + rsltURI);

        var tmpTxt = '';
        tmpTxt += 'baseUrl      : ' + document.baseURI + '\n\n';
        tmpTxt += 'urlParams    : ' + urlParams + '\n';
        tmpTxt += 'rsltURI      : ' + rsltURI + '\n';
        tmpTxt += 'fcBkUrl      : ' + fcBkUrl + '\n';
        tmpTxt += 'gsprUrl      : ' + gsprUrl + '\n';
        // alert(tmpTxt);

        $("#extLinkDisplay").prop("href", rsltURI ).prop("target", "_blank").text("Der Lesezeichen-Link");
